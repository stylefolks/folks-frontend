...
- [x] 크루 명 검색 가능하도록 Input
  - [ ] 디바운스 처리 필요
- [ ] Latest Event
  - [ ] 크루에서 개최한 최신 이벤트 게시물 보여주기
  - [ ] 메인의 Crews와 같은 작은 카드 형태 UI(클릭 시, 해당 게시글로 이동)
- [x] 크루 전체 보여주기
...
- [ ] 작성자 보여주기 & 작성날짜(createdAt, updatedAt이 있는 경우 updatedAt으로 보여주고 편집됨 표기)
  - [ ] 작성자 클릭 시 해당작성자 프로필 (profile/유저아이디)로 이동
...
- [ ] 게시글 리스트 보여주기
  ...
  - [ ] 프로필 클릭 시 해당 유저 프로필 페이지로 이동 (profile/유저아이디)
  - [ ] 내 덧글이면 수정 & 삭제 가능하게 UI
- [x] profile이미지 & Input창 & 등록 버튼
- [ ] TODO: 덧글 타이핑 시 리렌더 막기
...
- [ ] 미 가입인 경우 join버튼 가입인 경우 dismiss버튼 노출 (크루 description우측으로 이동 시키고 작은 아이콘으로 변경하기)
  ...
  - [x] Event : 게시글은 좌측 작은 이미지 중간 타이틀 우측 덧글 개수 및 좋아요 개수 간단하게 렌더 (나중에 위치 멘션 넣으면 위치도 렌더되도록 하자)
...
- [x] 본인 페이지로 이동 시 Nav 밑에 설정 버튼 제공
  - [x] 로그아웃, Settings 등
- [x] 팔로워 팔로잉 개수
- [ ] 팔로워 팔로잉 클릭 시 해당 유저 리스트 보여주는 모달 UI(삭제 차단 등은 MVP 미지원, glass UI)
- [x] 가입한 크루 리스트
  - [x] 클릭 시 해당 크루로 이동
- [x] 본인이 작성한 게시글 리스트
